...
class ProcessCallBack(BaseCallBack):
    ...
    def after_running_batch(self):
        ...
        current_epoch = registry.get("current_epoch")
        current_batch = registry.get("current_batch")
        if self.every_n_batches and current_batch % self.every_n_batches == 0 and\
            self.every_n_epoch and current_epoch % self.every_n_epoch == 0:
            self.progress_meter.display(current_epoch, current_batch, self.logger)
...
class ProgressMeter:
    ...
    def display(self, epoch_i, batch_i, logger):
        ...
        percent = math.ceil(100.0 * (batch_i + 1) / self.batch_n)
        speed = 0.0  # 默认值
        ...
        entries += "Epoch " + self.epoch_fmtstr.format(epoch_i + 1) + ": "
        entries += f"{percent:.0f}% | "
        entries += f"{bar}| ",
        entries += self.batch_fmtstr.format(batch_i + 1)
